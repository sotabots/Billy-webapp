...
export type TPayoffMethodType = 'bank' | 'crypto'

export type TPayoffMethod = {
  title: string
  type: TPayoffMethodType
  image: string
  is_bank_ru?: boolean
  fields: {
    [key: string]: { // TUserPayoffMethod.label: wallet_address | payment_link | iban | email | recipient_name
      type: 'string' | 'boolean'
      is_optional: boolean
    }
  }
